#!/usr/bin/env python3
"""
Benchmark all MTP optimizations on Qwen3.5-35B-A3B.
...
Compares:
  ...
  5. Shared-expert-only self-speculative decoding
"""
...
MAX_TOKENS = 128
NUM_RUNS = 2  # runs per prompt for averaging
...
# Prompts designed to test different optimization strengths
PROMPTS = {
    "code": [
    ...
    "summarization": [
        # These prompts have repetitive content that prompt lookup can exploit
...
def prompt_lookup_generate(
    ...
    while len(tokens) < max_tokens and tid not in eos_set:
        total_steps += 1
        # Try prompt lookup draft
        draft_ids = drafter.draft(tokens)

        if not draft_ids:
            # No match — regular autoregressive step
            logits = model(tok.reshape(1, 1), cache=cache)
            mx.eval(logits)
            tok = mx.argmax(logits[:, -1, :], axis=-1)
            mx.eval(tok)
        ...
        if accepted == len(draft_ids) + 1:
            ...
            n_to_replay = accepted + 1  # accepted drafts + the rejected token
            replay = mx.array([tokens[-(n_to_replay + 1)]] + tokens[-n_to_replay:])
            replay_logits = model(replay.reshape(1, -1), cache=cache)
...
def shared_expert_generate(
    ...
    """Generate with shared-expert-only self-speculation.

    Draft tokens by running the full model with only shared experts in MoE layers
    (skipping the expensive top-k routing through 256 experts). Verify drafts
    with the full model.
    """
    cache = make_prompt_cache(model)
    from vllm_mlx_mtp.cache_utils import save_cache_state, restore_cache_state
    ...
    while len(tokens) < max_tokens and tid not in eos_set:
        ...
        # Save cache before drafting
        saved = save_cache_state(cache)

        # Draft using shared-expert-only mode
        draft_ids = []
        ...
        if not draft_ids:
            # No drafts — restore cache and do regular step
            restore_cache_state(cache, saved)
            logits = model(tok.reshape(1, 1), cache=cache)
            mx.eval(logits)
            tok = mx.argmax(logits[:, -1, :], axis=-1)
        ...
        # Restore cache and verify all drafts in a single batch pass
        restore_cache_state(cache, saved)
        verify_input = mx.array([[tid] + draft_ids])
        verify_logits = model(verify_input, cache=cache)
        mx.eval(verify_logits)
        ...
        if accepted == len(draft_ids) + 1:
            ...
            # Partial accept — restore and replay accepted tokens + correction
            restore_cache_state(cache, saved)
...
def run_benchmark(model, tokenizer, eos_set, config, mtp_head_bf16, mtp_head_q4):
    """Run all benchmarks."""
    results = []
    ...
    for cat in categories:
        for pidx, prompt in enumerate(PROMPTS[cat]):
            ...
            def avg_runs(fn, n=NUM_RUNS):
                """Run fn multiple times, return best tokens/time."""
                best_tps = 0
                best_result = None
                for _ in range(n):
                    r = fn()
            ...
            # 5. Shared-expert-only self-speculation (draft 1)
            if shared_drafter:
            ...
            # 6. Shared-expert-only self-speculation (draft 3)
            if shared_drafter:
            ...
            # Progress
            bf16_ratio = bf16_tps / base_tps if base_tps > 0 else 0
            q4_ratio = q4_tps / base_tps if base_tps > 0 else 0
            ...
            print(f"    [{cat}:{pidx}] base={base_tps:.0f} bf16={bf16_ratio:.2f}x q4={q4_ratio:.2f}x pl={pl_ratio:.2f}x{se_info} '{preview}'")
...
def print_report(results: List[Result]):
    ...
    print()
    print("=" * 110)
    print("OPTIMIZATION BENCHMARK REPORT — Qwen3.5-35B-A3B-4bit")
    print("=" * 110)
    print()
    ...
    print()
    print("PER-CATEGORY ACCEPTANCE RATES")
    print("-" * 90)
    header = f"{'Method':<20}"
    for cat in categories:
        header += f" {cat:>12}"
    ...
    for method in methods:
        ...
        for cat in categories:
            cat_method = [r for r in results if r.method == method and r.category == cat]
            if cat_method:
                avg_acc = sum(r.acceptance for r in cat_method) / len(cat_method)
                line += f" {avg_acc:>11.0%}"
            else:
                line += f" {'n/a':>12}"
        print(line)
...
def main():
    print("=" * 80)
    print("MTP Optimization Benchmark — Qwen3.5-35B-A3B-4bit")
    print("=" * 80)
    ...
    bf16_params = sum(v.nbytes for _, v in mlx.utils.tree_flatten(mtp_head_bf16.parameters()))
    q4_params = sum(v.nbytes for _, v in mlx.utils.tree_flatten(mtp_head_q4.parameters()))
    print(f"  MTP head BF16: {bf16_params / 1e6:.1f} MB")
    print(f"  MTP head Q4:   {q4_params / 1e6:.1f} MB ({q4_params / bf16_params:.0%} of BF16)")

    # Run benchmarks
    print("\nRunning benchmarks...")
    results = run_benchmark(model, tokenizer, eos_set, config, mtp_head_bf16, mtp_head_q4)

    # Print report
    print_report(results)

    # Save results
    output_file = "benchmark_optimizations_35B.json"
